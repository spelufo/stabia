save_cell(scan::HerculaneumScan, cell::AbstractArray{Gray{N0f16},3}, jy::Int, jx::Int, jz::Int) = begin
  path = cell_path(scan, jy, jx, jz)
  mkpath(dirname(path))
  save(path, cell)
end

save_cells(scan::HerculaneumScan, cells::AbstractArray{Gray{N0f16},5}, jys::UnitRange{Int}, jxs::UnitRange{Int}, jz::Int) = begin
  for jy in jys, jx in jxs
    cell = @view cells[:, :, :, jy - jys.start + 1, jx - jxs.start + 1]
    save_cell(scan, cell, jy, jx, jz)
  end
end

load_grid_cell_from_slices(scan::HerculaneumScan, jy::Int, jx::Int, jz::Int) = begin
  cell = zeros(Gray{N0f16}, (CELL_SIZE, CELL_SIZE, CELL_SIZE))
  izs = cell_range(jz, scan.slices)
  iys = cell_range(jy, scan.height)
  ixs = cell_range(jx, scan.width)
