...
build_small_volume(scan::HerculaneumScan; from=:server) = begin
  ...
  Base.GC.gc()
  println("Done loading tiffs. Saving...")
  # Using TiffImages.save() instead of the generic save() reduces memory usage
  # dramatically. I was always getting OOM Killed with the regular save.
  TiffImages.save(small_volume_path(scan), vol)
end
